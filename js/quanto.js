...
function Graph(graphJson) {
  ...
  this.dirEdges = d3.map(graphJson.dir_edges);
...
function drawGraph(trans, thy, graph, svg, inserted, removed) {
  ...
  graph.dirEdges.values().forEach(function(e) {
    var sc = trans.toScreen(graph.src(e).annotation.coord);
    var tc = trans.toScreen(graph.tgt(e).annotation.coord);
    svg.append("line")
       .attr("class", "edge")
  ...
  graph.nodeVertices.forEach(function(k,v) {
    ...
    var w = trans.scaleToScreen(0.4);
    var h = trans.scaleToScreen(0.4);
...
d3.json("red_green.qtheory", function(errorThy, thyJson) {
  ...
  d3.selectAll(".qderive-result")
    .each(function() {
      ...
      d3.json(jsonFile, function(errorGr, derJson) {
        ...
        addGraph(thy, der.root, div1);
        addGraph(thy, der.steps[der.steps.length-1].graph, div2);
      });
    });
  ...
  d3.selectAll(".qsynth")
    .each(function() {
      var div = d3.select(this);
      var jsonFile = thy.name + "/" + div.attr("data-src") + ".qsynth";
      d3.json(jsonFile, function(errorGr, synthJson) {
        div.append("div").html("synth with " + synthJson.classes.length + " eqclasses")
        
        for (var i = 0; i < 100; i++) {
          var cls = synthJson.classes[i];
          div.append("hr");
          div.append("h2").html("Class: " + i)
          var div1 = div.append("div");
          div1 = div1.append("div").attr("class", "qgraph block");
          addGraph(thy, new Graph(cls.rep), div1);
          div.append("span").html(cls.data);
          div.append("h3").html("congruences");
          for (var key in cls.congs) {
            var div1 = div.append("div").attr("class", "qsynth block");
            addGraph(thy, new Graph(cls.congs[key]), div1);
          }
          div.append("h3").html("redexes");
          for (var key in cls.redexes) {
            var div1 = div.append("div").attr("class", "qsynth block");
            addGraph(thy, new Graph(cls.redexes[key]), div1);
          }
        }
        // synthJson.classes.forEach(function(cls) {
